...
class Brent extends App {
  ...
  val DF = spark.read.option("header", "true").option("inferSchema", "true").csv("file:///root/GIT/lamastex/spark-trend-calculus/src/main/resources/brent.csv").filter(year(col("DATE")) >= 2015)
  DF.show
